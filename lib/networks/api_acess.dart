...
import 'package:genzeh911/features/auth/data/rx_set_password/rx.dart';
...
OnboardRx onboardRx = OnboardRx(empty: OnboardModel(), dataFetcher: BehaviorSubject<OnboardModel>());

SignupRx signupRx = SignupRx(empty: SignupModel(), dataFetcher: BehaviorSubject<SignupModel>());

LoginRx loginRx =LoginRx(empty: LoginModel(), dataFetcher: BehaviorSubject<LoginModel>());

GetLogoutResponseRX getLogoutResponseRX = GetLogoutResponseRX(empty: {}, dataFetcher: BehaviorSubject<Map>());

OtpSendRx otpSendRx = OtpSendRx(empty: {}, dataFetcher: BehaviorSubject<Map>());

OtpVerifyRx otpVerifyRx = OtpVerifyRx(empty: {}, dataFetcher: BehaviorSubject<Map>());

SetPasswordRx setPasswordRx =  SetPasswordRx(empty: Map, dataFetcher: BehaviorSubject<Map>());

InsightRx insightRx = InsightRx(empty: InsightModel(), dataFetcher: BehaviorSubject<InsightModel>());

InsightDetailsRx insightDetailsRx = InsightDetailsRx(empty: SingleInsightModel(),dataFetcher: BehaviorSubject<SingleInsightModel>());

GetProfileRx profileRx = GetProfileRx(empty: ProfileModel(), dataFetcher: BehaviorSubject<ProfileModel>());

GetEditProfileResponseRX getEditProfileResponseRX =GetEditProfileResponseRX(empty: {}, dataFetcher: BehaviorSubject<Map>());

ScanResultRx scanResultRx = ScanResultRx(empty: ScanResultModel(), dataFetcher: BehaviorSubject<ScanResultModel>());

ScanHistoryRx scanHistoryRx = ScanHistoryRx(empty: ScanHistoryModel(),dataFetcher: BehaviorSubject<ScanHistoryModel>());

ScanDataRx scanDataRx =ScanDataRx(empty: ScanModel(), dataFetcher: BehaviorSubject<ScanModel>());

ScanDatDetailsaRx scanDatDetailsaRx = ScanDatDetailsaRx(empty: HistoryDetailsModel(),dataFetcher: BehaviorSubject<HistoryDetailsModel>());
